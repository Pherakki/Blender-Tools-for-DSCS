...
import numpy as np
from ...dataproc.interpolation import lerp
from ...dataproc.rotation import euler_to_quat
...
def extract_fcurves(action):
    ...
    for fcurve in action.fcurves:
        ...
        # Get datapath/array_index key
        fcurve_data = res[data_path]
        array_index = fcurve.array_index
        
        # Extract data
        fcurve_co = np.empty(len(fcurve.keyframe_points)*2)
        fcurve.keyframe_points.foreach_get("co", fcurve_co)
        fcurve_data[array_index] = {int(k): v for k, v in zip(fcurve_co[0::2], fcurve_co[1::2])}
...
def synchronized_transforms_from_fcurves(fcurves):
...
def synchronized_bone_transforms_from_fcurves(fcurves_dict):
    bone_fcurves = bone_fcurves_from_fcurves(fcurves_dict)
    synchronized_transforms_from_fcurves(bone_fcurves.values())
    return bone_fcurves


def synchronized_object_transforms_from_fcurves(fcurves_dict):
    obj_fcurves = object_transforms_from_fcurves(fcurves_dict)
    synchronized_transforms_from_fcurves([obj_fcurves])
    return obj_fcurves


def synchronized_quat_bone_transforms_from_fcurves(fcurves_dict, bones):
    bone_fcurves = synchronized_bone_data_from_fcurves(fcurves_dict)
...
def synchronized_quat_object_transforms_from_fcurves(fcurves_dict, obj):
    obj_fcurves = synchronized_object_transforms_from_fcurves(fcurves_dict)
...
def interpolate_keyframe(frame_idxs, frame_values, idx, interpolation_function):
    ...
    if next_largest_frame == next_smallest_frame:
        return frame_values[next_smallest_frame]
    else:
        t = (idx - next_smallest_frame) / (next_largest_frame - next_smallest_frame)

    # TODO: Should change lerp to the proper interpolation method
...
def produce_interpolation_method(frame_idxs, frame_values, default_value, interpolation_function):
    """
    Returns an interpolation function dependent on the number of passed frames.
    """
    if len(frame_idxs) == 0:
