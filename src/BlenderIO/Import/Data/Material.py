...
import bpy
# from .....external.blunger.Text.encoding import logged_decode


def import_materials(material_names, gi, path, errorlog, rename_imgs, use_custom_nodes):
    texture_bank = {}
    import_images(gi, texture_bank, path)
    
    materials = []
    name_lookup = {}
    mat_names = iter(material_names)
    for material in gi.materials:
        ...
        add_flag(bool((material.flags >>  0) & 0x01), "flag_0")
        add_flag(bool((material.flags >>  1) & 0x01), "cast_shadow")
        add_flag(bool((material.flags >>  2) & 0x01), "flag_2")
        add_flag(bool((material.flags >>  3) & 0x01), "flag_3")
        add_flag(bool((material.flags >>  4) & 0x01), "flag_4")
        add_flag(bool((material.flags >>  5) & 0x01), "flag_5")
        add_flag(bool((material.flags >>  6) & 0x01), "flag_6")
        add_flag(bool((material.flags >>  7) & 0x01), "flag_7")
        add_flag(bool((material.flags >>  8) & 0x01), "flag_8")
        add_flag(bool((material.flags >>  9) & 0x01), "flag_9")
        add_flag(bool((material.flags >> 10) & 0x01), "flag_10")
        add_flag(bool((material.flags >> 11) & 0x01), "flag_11")
        add_flag(bool((material.flags >> 12) & 0x01), "flag_12")
        add_flag(bool((material.flags >> 13) & 0x01), "flag_13")
        add_flag(bool((material.flags >> 14) & 0x01), "flag_14")
        add_flag(bool((material.flags >> 15) & 0x01), "flag_15")
