...
def mk_util_attribute_node(nodes, attr_name, name, location, parent):
    # if len(attr_name) > 18:
    #      raise ValueError(f"Attribute name too long: {attr_name}")
    return mk_attribute_node(nodes, "OBJECT", f"DSCS_UtilProperties.{attr_name}", name, location, parent)
