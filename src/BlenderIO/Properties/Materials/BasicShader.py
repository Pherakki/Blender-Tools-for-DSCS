...
def rebuild_basic_tree(bpy_material, used_images):
    ...
    # Samplers
    if props.color_sampler.active:
        color_sampler = sg.texture_sampler("ColorSampler", used_images.get(props.color_sampler.typename))
        
    if props.clut_sampler.active:
        clut_sampler = sg.texture_sampler("CLUTSampler", used_images.get(props.clut_sampler.typename))
...
#         generate_material_nodes(model_data, new_material, IF_material)
...
#                     new_material.alpha_threshold = 0.
#                 else:
#                     raise Exception(f"Unknown GL_ENUM \'{hex(gl_enum)}\' encounted in glAlphaTest.")
...
#             raise Exception(f"Unable to load texture: {e}") from e
