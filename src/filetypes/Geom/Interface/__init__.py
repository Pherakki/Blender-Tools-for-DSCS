...
# from ....serialization.DSCSTargets import DSCSOffsetCalculator
from ..Binary import GeomFileBinaryDSCSOpenGL, GeomFileBinaryDSCSPS, GeomFileBinaryMegido72
from ..Binary.Camera import CameraBinary
from ..Binary.Light import LightBinary
from ..Binary.Mesh.Base import VertexAttributeBinary
from ..Binary.Material import MaterialBinary, ShaderUniformBinary, OpenGLSettingBinary
...
class GeomFile:
    ...
    __KEY_MAPPING = {
        "DSCS_OpenGL": GeomFileBinaryDSCSOpenGL,
        "DSCS_PS":   GeomFileBinaryDSCSPS,
        "Megido72":  GeomFileBinaryMegido72
    }
    ...
    def from_file(cls, path, model_type):
        return cls._from_file(cls.binary_type(model_type))
    
    @classmethod
    def from_dscs_opengl_file(cls, path):
        return cls._from_file(path, GeomFileBinaryDSCSOpenGL)
    
    @classmethod
    def from_dscs_ps_file(cls, path):
        return cls._from_file(path, GeomFileBinaryDSCSPS)
    
    @classmethod
    def from_megido72_file(cls, path):
        return cls._from_file(path, GeomFileBinaryMegido72)
    
    @classmethod
    def _from_file(cls, path, binary_class):
        binary = binary_class()
        binary.read(path)
    ...
    def to_binary(self, model_type, invalidate_self_allowed=False):
        ...
        binary.textures   = [t.encode('utf8')[:0x20].ljust(0x20, b'\x00') for t in self.textures]
        ...
        # # Offsets
        # ot = DSCSOffsetCalculator()
        # binary.rw_header(ot)
        # if binary.mesh_count:
        #     binary.meshes_offset = ot.tell()
        #     ot.rw_objs(binary.meshes, binary.MESH_TYPE, binary.mesh_count)
        #     for mesh in binary.meshes:
        #         mesh.vertices_offset          = ot.tell() if mesh.vertex_count            else 0; mesh.rw_VAO(ot)
        #         mesh.matrix_palette_offset    = ot.tell() if mesh.matrix_palette_count    else 0; mesh.rw_matrix_palette(ot)
        #         mesh.indices_offset           = ot.tell() if mesh.index_count             else 0; mesh.rw_IBO(ot)
        #         ot.align(ot.tell(), 0x04)
        #         mesh.vertex_attributes_offset = ot.tell() if mesh.vertex_attribute_count  else 0; mesh.rw_vertex_attributes(ot)
        # else:
        #     binary.meshes_offset = 0

        # binary.materials_offset     = ot.tell() if binary.material_count         else 0; binary.rw_materials(ot)
        # binary.textures_offset      = ot.tell() if binary.texture_section_size   else 0; binary.rw_textures(ot)
        # binary.light_sources_offset = ot.tell() if binary.light_source_count     else 0; binary.rw_lights(ot)
        # binary.cameras_offset       = ot.tell() if binary.camera_count           else 0; binary.rw_cameras(ot)
        # ot.align(ot.tell(), 0x10)
        # binary.ibpms_offset         = ot.tell() if binary.ibpm_count             else 0; binary.rw_ibpms(ot)
        # binary.extra_clut_offset    = ot.tell() if binary.extra_clut is not None else 0; binary.rw_extra_clut(ot)
        return binary
...
class Uniform:
    ...
    def to_binary(self, float_count):
        binary = ShaderUniformBinary()
        binary.index = self.index
        binary.float_count = float_count
        binary.payload = binary.pack(self.data)
        return binary


class ShaderUniform(Uniform):
    is_texture = False
    ...
    def to_binary(self):
        return super().to_binary(len(self.data))
